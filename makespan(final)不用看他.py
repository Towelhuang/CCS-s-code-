...
data = pd.read_csv(r"C:\Users\mb002b\Desktop\DQNtest\gantt chart.csv")
...
def makespan(list):
	...
	STK_Vol = np.ones(W, dtype = int)*STK_Max
	#根據openedSTKID來產生openedSTK
	openedSTK[openedSTKID] = 1
	STK_Vol[openedSTKID] = 0
	...
	for i in range(1,N):
		...
		#需要暫存
				...
				if(Overload == False):
					...
					STK = STK_avi[np.argmin(abs(STK_avi-U[i]))] #STK=這個工單要去的STK
					if (STK > W-1):
						STK = STK-W
					#print('J%s-%s-STK%s-時間%s'%(pi[h],i,STK,T[U[i-1]][STK]*2))
					InSTK[STK].append(c[i-1][pi[h]]+T[U[i-1]][STK])
					...
					if(BackST < past_Csm[U[i]][machine[i][pi[h]]]):  #會停留在STK不能馬上被傳送回工作站
						...
						past_Csm[U[i]][machine[i][pi[h]]] = C_sm[U[i]][machine[i][pi[h]]]
						OutSTK[STK].append(s[i][pi[h]]-T[U[i]][STK])
	...
	plt.yticks(np.arange(100),ylabelname)
	#工單數10個
